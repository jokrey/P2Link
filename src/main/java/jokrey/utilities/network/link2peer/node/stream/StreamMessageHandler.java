...
public class StreamMessageHandler {
    ...
    public void receivedPart(P2LNodeInternal parent, ReceivedP2LMessage message) throws IOException {
        ...
        if(stream != null)
            ...
            //to-do - this default handling is technically a ddos issue - since the sender ip can be spoofed and we will send this package to anyone.
            //to-do - also we will send a package for a received package without knowledge of whether that is valid.
            ...
            parent.sendInternalMessage(message.sender, defaultReceipt); //send default receipt in case our stream has already unregistered after close, but the receipt got lost....
        }
...
//        return (P2LOrderedInputStream) inputStreams.computeIfAbsent(identifier, k -> new P2LOrderedInputStreamImplV1(parent, from, type, conversationId));
    ...
    public void receivedReceipt(ReceivedP2LMessage rawReceipt) {
        ...
        if(stream != null)
            ...
            System.out.println("received receipt message for unknown out stream("+new SenderTypeConversationIdStepIdentifier(rawReceipt)+")"); //default handling?
    }
