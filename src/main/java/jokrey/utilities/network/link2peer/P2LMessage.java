...
public final class P2LMessage {
     ...
     * Should be the same between all peers, as the receiver uses it to create the internal buffer and will cut messages longer than this limit.
     */
    ...
    public Hash getContentHash() {
        if(contentHash == null)
            contentHash = HeaderUtil.contentHashFrom(sender, raw, payloadLength); //no need for thread safety, same value computed in worst case
        return contentHash;
    }
    ...
    public byte[] asBytes() {
        if(payload == null) //no need for thread safety measures since the same value is calculated...
            payload = Arrays.copyOfRange(raw, HeaderUtil.HEADER_SIZE, HeaderUtil.HEADER_SIZE+payloadLength);
        return payload;
    }
    ...
    private P2LMessage(String sender,
        ...
        if(raw.length > CUSTOM_RAW_SIZE_LIMIT)
            throw new IllegalArgumentException("total size of raw cannot exceed "+CUSTOM_RAW_SIZE_LIMIT+", user set limit - size here is: "+raw.length+" - max payload size is: "+(CUSTOM_RAW_SIZE_LIMIT-HeaderUtil.HEADER_SIZE));
    ...
    public static P2LMessage fromPacket(DatagramPacket packet) {
        ...
        return new P2LMessage(WhoAmIProtocol.toString(packet.getSocketAddress()), type, conversationId, requestReceipt, isReceipt, expirationTimeoutInSeconds, null, raw, packet.getLength()-HeaderUtil.HEADER_SIZE, null);
    }
    ...
    public boolean payloadEquals(byte[] o) {
        ...
        for(int i=0;i<o.length;i++)
            if(raw[HeaderUtil.HEADER_SIZE+i] != o[i])
                return false;
        return true;
    }
    ...
    public P2LMessage mutateToRequestReceipt() {
        HeaderUtil.writeHeader(raw, type, conversationId, true, isReceipt, expirationTimeoutInSeconds);
        return new P2LMessage(sender, type, conversationId, true, isReceipt, expirationTimeoutInSeconds, contentHash, raw, payloadLength, payload);
    }
    ...
    public static class Factory {
         ...
         * @param type message type - used to efficiently distinguish messages, allowing applications to wait for them
         * @param payload the payload of the message. The payload can be anything, but it's size cannot not exceed {@link #MAX_UDP_PACKET_SIZE} - {@link HeaderUtil#HEADER_SIZE}.
         * @return the new message
         */
        ...
        public static P2LMessage createSendMessageWith(int type, int conversationId, short expirationTimeoutInSeconds, int totalPayloadSize, byte[]... payloads) {
            byte[] raw = new byte[HeaderUtil.HEADER_SIZE + totalPayloadSize];
            HeaderUtil.writeHeader(raw, type, conversationId, false, false, expirationTimeoutInSeconds);
            int index = HeaderUtil.HEADER_SIZE;
            ...
            return new P2LMessage(null, type, conversationId, false, false, expirationTimeoutInSeconds, null, raw, totalPayloadSize, payloads.length==1?payloads[0]:null); //sender does not need to be set on send messages - it is automatically determined by the received from the ip header of the packet
        }
        ...
        public static P2LMessage createBroadcast(String sender, int brdMsgType, byte[] payload) {
            byte[] raw = new byte[HeaderUtil.HEADER_SIZE + payload.length];
            HeaderUtil.writeHeader(raw, brdMsgType, 0, false, false, MAX_TIMEOUT);
            System.arraycopy(payload, 0, raw, HeaderUtil.HEADER_SIZE, payload.length);
            return new P2LMessage(sender, brdMsgType, P2LNode.NO_CONVERSATION_ID,false, false, MAX_TIMEOUT, null, raw, payload.length, payload);
        }
    }
    ...
    private int pointer=HeaderUtil.HEADER_SIZE;
    /** resets the internal iterating pointer */
    public void resetReader() {pointer = HeaderUtil.HEADER_SIZE;}
    ...
    public byte[] nextVariable() {
        long[] li_bounds = LIbae.get_next_li_bounds(raw, pointer, pointer, HeaderUtil.HEADER_SIZE + payloadLength - 1);
    ...
    public String asString() {
        return new String(raw, HeaderUtil.HEADER_SIZE, payloadLength, StandardCharsets.UTF_8);
    }
    ...
    private static class HeaderUtil {
        ...
        private static Hash contentHashFrom(String sender, byte[] raw, int payloadLength) {
            //fixme speed ok?
            try {
                ...
                if(payloadLength > 0)
                    hashFunction.update(raw, HeaderUtil.HEADER_SIZE, payloadLength); //only payload
