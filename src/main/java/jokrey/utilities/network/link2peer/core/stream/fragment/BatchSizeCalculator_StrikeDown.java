...
public class BatchSizeCalculator_StrikeDown extends BatchSizeCalculator {
    public static BatchSizeCalculatorCreator DEFAULT_CREATOR = c -> new BatchSizeCalculator_StrikeDown(c, 200, 64, 4);
    ...
    @Override public void adjustBatchSize(LossResult lossResult) {
        switch (lossResult) {
            ...
            case ACCEPTABLE_LOSS:
                if(lastBsWithoutLoss==bs) lastBsWithoutLoss-=step_length; //this is weird - and barely explainable - used so that excessive loss does not instantly strike down, but can take a step down (if that step is still bad - THEN: STRIKE DOWN!
                break;
